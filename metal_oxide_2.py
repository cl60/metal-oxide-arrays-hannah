...
class SelfAssemblyModel:
    def __init__(self, N, width, length, k0, alpha, x_p, sigma, velocity_mag, dt):
        self.width = width
        self.length = length
    ...
    def step(self):
        ...
        for particle in self.particles:
            ...
            particle.update(self.time, self.k0, f_x, self.dt)
...
k0 = 1  # Base rate constant
alpha = 1  # Scaling factor for velocity correction
x_p = length/3  # Peak location for concentration profile
sigma = 1.0  # Spread of the peak
velocity_mag = 1  # Small velocity for particle motion
dt = 0.01

model = SelfAssemblyModel(N, width, length, k0, alpha, x_p, sigma, velocity_mag, dt)
